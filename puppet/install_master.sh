...
DATA_PATH=$THIS_DIR/Cloudbyte-CI-Data
OSEXT_PATH=$THIS_DIR/Cloudbyte-CI-Modules
OSEXT_REPO=https://github.com/CloudByteStorages/Cloudbyte-CI-Modules
...
# Clone or pull the the Cloudbyte-CI-Modules repository
if [[ ! -d $OSEXT_PATH ]]; then
    echo "Cloning Cloudbyte-CI-Modules repo..."
    git clone $OSEXT_REPO $OSEXT_PATH
fi

if [[ "$PULL_LATEST_OSEXT_REPO" == "1" ]]; then
    echo "Pulling latest Cloudbyte-CI-Modules repo master..."
    cd $OSEXT_PATH; git checkout master && sudo git pull; cd $THIS_DIR
fi

if [[ ! -e $DATA_PATH ]]; then
    echo "Enter the URI for the location of your config data repository. Example: https://github.com/CloudByteStorages/CloudByte-CI-Data"
    read data_repo_uri
...
if [[ ! -e "$DATA_PATH/etc/nodepool/nodepool.yaml.erb" ]]; then
    ...
    cp -f $DATA_PATH/etc/nodepool/nodepool.yaml.erb $OSEXT_PATH/puppet/modules/cloudbyte_ci_modules/templates/nodepool
fi
...
sudo puppet apply --verbose $PUPPET_MODULE_PATH -e "class {'cloudbyte_ci_modules::master': $CLASS_ARGS }"
