...
class cloudbyte_ci_modules::master (
  $vhost_name = $::fqdn,
  $data_repo_dir = '',
...
#Extra, not part of openstack upstream:
  ...
  if $manage_jenkins_jobs == true {
    ...
    file { '/etc/jenkins_jobs/config/macros.yaml':
      ...
      group  => 'root',
      mode   => '0755',
      content => template('cloudbyte_ci_modules/jenkins_job_builder/config/macros.yaml.erb'),
    ...
    if $enable_fc != false {
      file { '/etc/jenkins_jobs/config/macros-fc.yaml':
        ...
        group  => 'root',
        mode   => '0755',
        content => template('cloudbyte_ci_modules/jenkins_job_builder/config/macros-fc.yaml.erb'),
        notify  => Exec['jenkins_jobs_update'],
      }
    }
  ...
  class { 'cloudbyte_ci_modules::zuul':
...
#    mode    => '0400',
    mode    => '0660',
    content => template("cloudbyte_ci_modules/nodepool/nodepool.yaml.erb"),
    require => [
      File['/etc/nodepool'],
      User['nodepool'],
    ],
  ...
  file { "/etc/profile.d/set_nodepool_env.sh":
      ...
      group  => 'root',
      mode   => '0755',
      content => template('cloudbyte_ci_modules/nodepool/set_nodepool_env.sh.erb'),
  }
  ...
  file { "/etc/sudoers.d/90-nodepool-http-proxy":
      ...
      group  => 'root',
      mode   => '0440',
      source => 'puppet:///modules/cloudbyte_ci_modules/sudoers/90-nodepool-http-proxy',
  }
  ...
  file {"/var/lib/jenkins/credentials.xml":
      ...
      group  => 'jenkins',
      mode   => '0644',
      content => template('cloudbyte_ci_modules/jenkins/credentials.xml.erb'),
  }
  ...
  file {"/var/lib/jenkins/be.certipost.hudson.plugin.SCPRepositoryPublisher.xml":
      ...
      group  => 'jenkins',
      mode   => '0644',
      content => template('cloudbyte_ci_modules/jenkins/be.certipost.hudson.plugin.SCPRepositoryPublisher.xml.erb'),
  }
